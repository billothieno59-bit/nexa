...
def test_invoke_open_skill_succeeds():
    ...
    result = invoke_skill(
        caller_id="anyone", skill_id="test.echo", requested_intent="GENERAL.QUERY",
        registry=registry, text="hello",
    )
...
def test_invoke_locked_skill_denied():
    ...
    result = invoke_skill(
        caller_id="anyone", skill_id="test.locked", requested_intent="GENERAL.QUERY",
        registry=registry,
    )
...
def test_invoke_unknown_skill_denied():
    registry = make_registry_with_open_skill()
    result = invoke_skill(
        caller_id="anyone", skill_id="nonexistent", requested_intent="GENERAL.QUERY",
        registry=registry,
    )
    assert result.status == "denied"
...
def test_handler_exception_becomes_error_status():
    registry = SkillRegistry()
    manifest = SkillManifest(
        skill_id="test.broken", name="Broken", description="Raises.",
        tier="builtin", required_permissions=(),
    )
    ...
    result = invoke_skill(
        caller_id="anyone", skill_id="test.broken", requested_intent="GENERAL.QUERY",
        registry=registry,
    )
